...
impl ContigTile {
    fn new() -> ContigTile {
        ContigTile {contig_ranges: Vec::new()}
    }
}
...
impl ContigIndex {
    ...
    #[allow(dead_code)]
    fn get_or_insert_contig(&mut self,
                            contig_name: &str) -> &mut Contig { 
        ...
        if !self.contig_lookup.contains_key(contig_name) {
          ...
          self.contig_lookup.insert(contig_name.to_string(), contig_idx);
    ...
    fn add_contig_range(
        ...
        //if !self.contig_lookup.contains_key(contig_name) {
        //  contig_idx = self.contigs.len() as u32;
        //  self.contigs.push(Contig {
        //     name: contig_name.to_string(),              
        //     contig_tiles: Vec::new(),
        //  });
        //  self.contig_lookup.insert(contig_name.to_string(), contig_idx);
        //}else {
        //  contig_idx = *self.contig_lookup.get(contig_name).unwrap();
        //}
        //let mut contig = &mut self.contigs[contig_idx as usize];
        let contig = self.get_or_insert_contig(contig_name);
        //println!("end={}, last={}, len={}", end_bp, last_tile_idx, contig.contig_tiles.len());
        // Initialize any missing tiles up and including last_tile_idx
    ...
    fn init_search(&mut self, file_path: &str) {
      ...
      if f_ver != FORMAT_VERSION {
        println!("Incompatible file version {}, expected {}", f_ver, FORMAT_VERSION);
      }
      ...
      file.read_u32_into::<LittleEndian>(&mut self.tile_counts).unwrap();
      ...
      for n in 0..self.contig_count {
        ...
        //println!("adding >{}< as {} len = {}", string, n, string.len());
      ...
      for i in 0..file_count {
          ...
          let value1 = f64::from_le_bytes([
                buffer[start], buffer[start + 1], buffer[start + 2],
                buffer[start + 3], buffer[start + 4], buffer[start + 5],
                buffer[start + 6], buffer[start + 7], ]);

          let value2 = u64::from_le_bytes([
                buffer[start + 8], buffer[start + 9], buffer[start + 10],
                buffer[start + 11], buffer[start + 12], buffer[start + 13],
                buffer[start + 14], buffer[start + 15], ]);

          //println!("found: {} {:?} {}", string, value1, value2);
    ...
    #[allow(dead_code)]
    fn read_tile(&mut self, file: &mut File, contig: u32, tile: usize) -> Result<ContigTile, &'static str> {
      ...
      println!("read_tile: expecting {} ranges, byte_pos {}", range_count, byte_pos);
      ...
      for i in 0..range_count {
        ...
        let value2 = u64::from_le_bytes([
                buffer[start + 12], buffer[start + 13], buffer[start + 14],
                buffer[start + 15], buffer[start + 16], buffer[start + 17],
                buffer[start + 18], buffer[start + 19], ]);

        let value3 = u64::from_le_bytes([
                buffer[start + 20], buffer[start + 21], buffer[start + 22],
                buffer[start + 23], buffer[start + 24], buffer[start + 25],
                buffer[start + 26], buffer[start + 27], ]);
    ...
    #[allow(dead_code)]
    fn search(&mut self, i_file: &mut File, bgz_dir: String, q_contig: String, q_start: u64, q_end: u64) -> Result<Vec<ContigRange>, &'static str> {
        ...
        println!("Query: {}:{}-{}  tiles:{} to {}", q_contig, q_start, q_end, start_tile, end_tile);
        let mut hits = 0;
        ...
        if range_count > 0 {
          let range_data = self.read_tile(i_file, q_contig_idx, start_tile).unwrap().contig_ranges;
          if range_data[0].start_bp < q_end {
              ...
              for range in ranges.iter().rev() {
                  //println!("RESULT0: {} : {} - {}  file({}) @ {}", q_contig, range.start_bp, range.end_bp, range.bed_idx, range.bgzf_pos );
                  ...
                  let bgz_file = format!("{}/{}", bgz_dir, self.bgz_files[range.bed_idx as usize].name);
                  let mut reader = File::open(&bgz_file).map(bgzf::Reader::new).unwrap();
                  reader.seek(bgzf::VirtualPosition::from(range.bgzf_pos)).unwrap();
          ...
          if end_tile > start_tile {
            ...
            for t_idx in (start_tile+1)..=end_tile {
              ...
              if range_count > 0 {
                let range_data = self.read_tile(i_file, q_contig_idx, t_idx).unwrap().contig_ranges;
                if range_data[0].start_bp < q_end {
                  ...
                  for r_idx in 0..range_count {
                      ...
                      if range_data[r_idx as usize].start_bp < q_end {
                          //println!("RESULT1: {} : {} - {} file({}) @ {}", q_contig, range_data[r_idx as usize].start_bp, range_data[r_idx as usize].end_bp, range_data[r_idx as usize].bed_idx, range_data[r_idx as usize].bgzf_pos );
                          //
                          let bgz_file = format!("{}/{}", bgz_dir, self.bgz_files[range_data[r_idx as usize].bed_idx as usize].name);
                          let mut reader = File::open(&bgz_file).map(bgzf::Reader::new).unwrap();
                          reader.seek(bgzf::VirtualPosition::from(range_data[r_idx as usize].bgzf_pos)).unwrap();
                          let mut line = String::new();
                          reader.read_line(&mut line).unwrap();
                          print!("{}",line);
                          hits += 1;
                          ...
                          println!("Breaking because {} >= {}", range_data[r_idx as usize].start_bp , q_end );
                          break;
                      }
                  }
                }
...
pub fn build_idx(filenames: Vec<String>, bgz_dir: String, mut contig_index: ContigIndex, index_file: String){
    ...
    for filename in filenames {
        ...
        let mod_time = modification_time.duration_since(SystemTime::UNIX_EPOCH).unwrap();
        // Obtain file size
        let file_size = metadata.len();
        println!("Indexing {} : {} mod_time={:?}, bytes={}", fidx, bgz_file, mod_time, file_size);
        contig_index.bgz_files.push( BGZFile { name: filename.clone(), mod_time: mod_time.as_secs_f64(), bytes: file_size });
      ...
      while reader.read_line(&mut line).unwrap() > 0  {
        //println!("{line:?}");
        // "2\t92250086\t92250387\tDF0001317.2\t100\t-\n"
        let fields: Vec<&str> = line.trim_end().split('\t').collect();
        //println!("Fields: {:?}", fields);
        
        contig_index.add_contig_range(fields[0], fidx, fields[1].parse::<u64>().unwrap(), fields[2].parse::<u64>().unwrap(), virt_pos );
    ...
    //let _ = contig_index.save_igd_format(&index_file);
...
pub fn search_idx(filenames: Vec<String>, bgz_dir: String, mut contig_index: ContigIndex, index_file: String){
    ...
    for fsfile in &f_lookup {
       println!("It appears that {} has been added since the index was created!", fsfile);
    }
